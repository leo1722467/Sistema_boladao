...
function createFormField(column, foreignKeys) {
  ...
  if (column.is_foreign_key && foreignKeys[column.name]) {
    ...
    loadForeignKeyOptions(window.__MODEL__, column.name).then(options => {
...
async function buildRelatedForm(model, modalCounter) {
  ...
  columns.forEach(column => {
    const fieldElement = createFormField(column, foreign_keys);
...
async function loadItem() {
  ...
  for (const col of editableColumns) {
    const fieldElement = createFormField(col, schema.foreign_keys || {});
