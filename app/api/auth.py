...
async def login(payload: LoginRequest, session: AsyncSession = Depends(get_db)) -> TokenResponse:
    ...
    try:
        ...
        logger.warning("/auth/login invalid credentials", extra={"email": payload.email})
...
async def token(form_data: OAuth2PasswordRequestForm = Depends(), session: AsyncSession = Depends(get_db)) -> TokenResponse:
    ...
    try:
        ...
        logger.exception("Unexpected error during /auth/token")
        raise
    return TokenResponse(access_token=access, refresh_token=refresh)


@router.post("/refresh", response_model=TokenResponse)
async def refresh(current_user: UserAuth = Depends(get_current_user), session: AsyncSession = Depends(get_db)) -> TokenResponse:
...
async def me(current_user: UserAuth = Depends(get_current_user), session: AsyncSession = Depends(get_db)) -> MeResponse:
